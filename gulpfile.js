...
gulp.task('sass', function () {
    // Cf-web
    var cfStream = gulp.src('src/cf.scss')
      .pipe(sass({
...
gulp.task('uglify', ['uglify:annotate'], function() {
  return gulp.src('dist/cf.min.js')
    .pipe(uglify())
    .pipe(gulp.dest('dist'));
});
...
gulp.task('uglify:annotate', function() {
  return gulp.src('dist/cf.js')
    .pipe(ngAnnotate())
    .pipe(rename('cf.min.js'))
    .pipe(gulp.dest('dist'));
});
 ...
 */
gulp.task('html2js:cf', function() {
  return gulp.src('src/ui/**/*.html')
    .pipe(ngHtml2Js({
      moduleName: 'cf-templates-html',
      rename: function(moduleName) {
        return '/cf.ui/' + moduleName;
      }
    }))
...
gulp.task('html2js:svg', function() {
  return gulp.src('src/svg/*.svg')
    .pipe(ngHtml2Js({
      moduleName: 'cf-templates-svg',
      rename: function(moduleName) {
        return '/cf.svg/' + moduleName;
      }
    }))
