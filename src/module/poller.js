...
angular.module('cf.services').provider('pollerSvc', function() {
  'use strict';
   ...
   */
  this.$get = function pollerFactory($q, $http, $timeout, _, CF_EVENT) {
    /* jshint unused:false */
      ...
      register: function(name, options) {
        ...
        _.defaults(options, settings, {
          ...
          catchMaxFail: function() {
            if (options.scope) {
              options.scope.$emit(CF_EVENT.POLL_ERROR);
            }
          },
        ...
        if (options.scope) {
          ...
          options.scope.$on(CF_EVENT.DOC_VISIBILITY_CHANGE,
              function(e, isHidden) {
            if (isHidden) {
